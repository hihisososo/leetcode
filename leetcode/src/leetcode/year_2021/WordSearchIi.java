...
public class WordSearchIi {
    ...
    private void makeTrie(char[][] board, Node root) {
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                ...
                addAvailableWords(board, "", i, j, wordSet, visit, memo);
            }
        }
    ...
    private void addAvailableWords(char[][] board, String prefix, int row, int col, HashSet<String> wordSet, boolean[][] visit, HashSet<String> memo) {
        ...
        String s = prefix + c;
        String memoStr = row + "/" + col + "/" + s;
        if (memo.contains(memoStr)) {
            return;
        }
        wordSet.add(s);
        memo.add(memoStr);
        ...
        if (row > 0) {
            addAvailableWords(board, s, row - 1, col, wordSet, cloneArray(visit), memo);
        }
        if (row < board.length - 1) {
            addAvailableWords(board, s, row + 1, col, wordSet, cloneArray(visit), memo);
        }
        if (col > 0) {
            addAvailableWords(board, s, row, col - 1, wordSet, cloneArray(visit), memo);
        }
